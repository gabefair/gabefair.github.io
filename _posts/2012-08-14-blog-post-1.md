...
Since `x` is a one-hot encoded vector, `h` is simply doing a lookup of the kth row of the weight matrix `W`




```python
def init_first_weights(len_vocab, dim_num_k):
    weights = np.random.randn(len_vocab, dim_num_k) #Initialising the matrices using a random samples of a normal distribution
    return np.matrix(weights) #Numpy Matrix; Initialised words vectors for each word in the vocabulary
```


```python
def forward_pass(weight_matrix_1,weight_matrix_2, x):
    h = np.dot(weight_matrix_1.T, x)
    u_c = np.dot(weight_matrix_2.T, h)
    y_c = self.softmax(u)
    return y_c, h, u
